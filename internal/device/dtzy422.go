...
func init() {
	RegDevice["DTZY422"] = &DTZY422{}
}

// NewDev ..
func (d *DTZY422) NewDev(id string, ele map[string]string) (Devicerwer, error) {
	ndev := new(DTZY422)
...
// GetElement ..
func (d *DTZY422) GetElement() (dict, error) {
	conn := dict{
		/***********************设备的特有的参数*****************************/
		"devaddr": d.devaddr,
		"commif":  d.commif,
...
// HelpDoc ..
func (d *DTZY422) HelpDoc() interface{} {
	conn := dict{
		"devaddr": "设备地址",
		/***********TOSHIBA设备的参数*****************************/
...
// CheckKey ..
func (d *DTZY422) CheckKey(ele dict) (bool, error) {
	return true, nil
}

/***************************************添加设备参数检验**********************************************/

func (d *DTZY422) crackbuf(inbuf []byte) (retbuf []byte) {
...
func (d *DTZY422) readCmd(taddr string, di []byte) []byte {
...
func (d *DTZY422) writeCmd(taddr string, di, value []byte) []byte {
	saddr := fmt.Sprintf("%012s", taddr)
	cmd := []byte{0x68}
	for i := 12; i >= 2; i -= 2 {
		temp, _ := strconv.Atoi(saddr[i-2 : i])
	...
	//cdi := d.plus33(di)
	cmdVal := append(di, value...)
	//cmd = append(cmd, []byte{0xE2, 0xCD, 0xA0}...)
	var bvi int
	for j, v := range cmdVal {
		if (j % 2) == 0 {
			bvi = int(v) + 0x33 + 0x48 + j
		} else {
			bvi = int(v) + 0x33 + 0x54 + j
		}
		if bvi > 255 {
			bvi -= 256
		}
...
func (d *DTZY422) rDateSum(data []byte) byte {
	sum := 0
...
func (d *DTZY422) rdvalue(rsport serial.Port, cmd int, ret map[string]interface{}) (err error) {
	var di []byte
	...
	for i := 0; i < 2; i++ {
		...
		if err == nil {
			...
			if d.rDateSum(results[startbyte:len-2]) == results[len-2] {
				...
				switch cmd {
				case 1:
					valb := d.crackbuf(results[startbyte+10 : startbyte+24])
					ret["总用电量"] = float64(Hex2Bcd(valb[5]))*10000 +
						float64(Hex2Bcd(valb[4]))*100 +
						float64(Hex2Bcd(valb[3])) +
						float64(Hex2Bcd(valb[2]))/100
					ret["剩余电量"] = float64(Hex2Bcd(valb[9]))*10000 +
						float64(Hex2Bcd(valb[8]))*100 +
						float64(Hex2Bcd(valb[7])) +
						float64(Hex2Bcd(valb[6]))/100
					ret["透支用电量"] = float64(Hex2Bcd(valb[13]))*10000 +
						float64(Hex2Bcd(valb[12]))*100 +
						float64(Hex2Bcd(valb[11])) +
						float64(Hex2Bcd(valb[10]))/100
				case 2:
					valb := d.crackbuf(results[startbyte+10 : startbyte+26])
					ret["报警电量"] = float64(Hex2Bcd(valb[3]))*100 +
						float64(Hex2Bcd(valb[2]))
					ret["报警负荷"] = float64(Hex2Bcd(valb[5])) +
						float64(Hex2Bcd(valb[4]))/100
					ret["允许透支金额"] = float64(Hex2Bcd(valb[7]))*100 +
						float64(Hex2Bcd(valb[6]))
					ret["允许囤积电量"] = float64(Hex2Bcd(valb[11]))*1000000 +
						float64(Hex2Bcd(valb[10]))*10000 +
						float64(Hex2Bcd(valb[9]))*100 +
						float64(Hex2Bcd(valb[8]))
				case 3:
				...
				case 4:
					...
					ret["购电次数"] = int(Hex2Bcd(valb[14]))*100 + int(Hex2Bcd(valb[13]))
				case 5:
					valb := d.crackbuf(results[startbyte+10 : startbyte+15])
					ret["控制字"] = int(valb[2])
					alarmStatus := map[int]string{
						0: "正常",
						1: "超负荷报警",
						2: "剩余电量低于报警电量",
						3: "超负荷报警,剩余电量低于报警电量",
						...
						5: "超负荷报警,超透支报警",
						6: "超负荷报警,剩余电量低于报警电量",
						7: "超负荷报警,剩余电量低于报警电量,超透支报警",
					}
					remaining := map[int]string{
						0: "欠电运行",
						1: "余电运行",
					}
					onoff := map[int]string{
						0: "ON",
						1: "OFF",
					}
					ret["运行状态"] = remaining[int(valb[3]&0x80/0x80)]
					ret["报警状态"] = alarmStatus[int(valb[3]&0xf)]
					ret["主开关"] = onoff[int(valb[4]&0x02/0x02)]
					ret["辅助开关"] = onoff[int(valb[4]&0x01)]
...
func (d *DTZY422) decodeVal(cmd string, val interface{}) (retb []byte, err error) {
	switch cmd {
	case "初始化":
...
func (d *DTZY422) wtvalue(rsport serial.Port, cmd string, value interface{}, ret map[string]interface{}) (err error) {
	var di []byte
	var bval []byte
...
// RWDevValue ..
func (d *DTZY422) RWDevValue(rw string, m dict) (ret dict, err error) {
	...
	if rw == "r" {
		err = d.rdvalue(rsport, 1, ret)
		err = d.rdvalue(rsport, 2, ret)
		err = d.rdvalue(rsport, 5, ret)
		if err != nil {
			log.Errorf("read DTZY422 faild %s", err.Error())
			return nil, err
		}
	} else {
		for k, v := range m {
